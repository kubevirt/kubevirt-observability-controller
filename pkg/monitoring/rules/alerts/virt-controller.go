	...
	return []promv1.Rule{
		...
		},
		{
			...
			Expr: intstr.FromString(fmt.Sprintf(
				"cluster:kubevirt_virt_controller_up:sum / on() "+
		...
		},
		{
			...
			Expr:  intstr.FromString(getErrorRatio(namespace, "virt-controller") + " >= 0.8"),
			For:   ptr.To(promv1.Duration("5m")),
			Annotations: map[string]string{
				summaryAnnotationKey: getRestCallsFailedWarning("virt-controller"),
			},
			Labels: map[string]string{
				severityAlertLabelKey:        "critical",
				operatorHealthImpactLabelKey: "critical",
