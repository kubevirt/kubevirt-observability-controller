...
func systemAlerts(_ string) []promv1.Rule {
	return []promv1.Rule{
		{
			Alert: "LowKVMNodesCount",
			Expr: intstr.FromString(
				"(cluster:kubevirt_nodes_allocatable:count > 1) and " +
					"(cluster:kubevirt_nodes_with_kvm:count < 2)",
			),
			For: ptr.To(promv1.Duration("5m")),
			Annotations: map[string]string{
				descriptionAnnotationKey: "Low number of nodes with KVM resource available.",
				summaryAnnotationKey: "At least two nodes with kvm resource required for " +
					"VM live migration.",
			},
		...
		},
		{
			...
			Expr: intstr.FromString(
				"((cluster:kubevirt_nodes_with_kvm:count or on() vector(0)) == 0 " +
					...
					" or (sum(kube_node_labels{label_kubevirt_io_schedulable='true'} " +
					"* on(node) group_left() (1 - kube_node_spec_unschedulable)) " +
					" or on() vector(0)) == 0",
			),
			For: ptr.To(promv1.Duration("5m")),
			Annotations: map[string]string{
