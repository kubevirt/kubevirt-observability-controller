...
var nodesRecordingRules = []operatorrules.RecordingRule{
	...
	},
	{
		...
		Expr: intstr.FromString(
			"count(count (kube_node_status_allocatable) by (node)) - cluster:kubevirt_non_schedulable_nodes:sum",
		),
	},
	{
		MetricsOpts: operatormetrics.MetricOpts{
		...
		MetricType: operatormetrics.GaugeType,
		Expr: intstr.FromString(
			"count(kube_node_status_allocatable{resource=\"devices_kubevirt_io_kvm\"} != 0) or vector(0)",
		),
	},
}
