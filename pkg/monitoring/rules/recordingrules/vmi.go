...
var vmiRecordingRules = []operatorrules.RecordingRule{
	...
	},
	{
		...
		MetricType: operatormetrics.GaugeType,
		Expr: intstr.FromString(
			"sum by (name, namespace) (clamp_min(kubevirt_vmi_guest_load_1m - vmi:kubevirt_vmi_vcpu:count, 0))",
		),
	},
	{
		MetricsOpts: operatormetrics.MetricOpts{
		...
		MetricType: operatormetrics.GaugeType,
		Expr: intstr.FromString(
			"sum by (name, namespace) (kubevirt_vmi_memory_usable_bytes) / " +
				"sum by (name, namespace) (kubevirt_vmi_memory_available_bytes)",
		),
	},
	{
		MetricsOpts: operatormetrics.MetricOpts{
