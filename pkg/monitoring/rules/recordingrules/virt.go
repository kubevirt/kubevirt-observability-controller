...
func virtRecordingRules(namespace string) []operatorrules.RecordingRule {
	return []operatorrules.RecordingRule{
		...
		},
		{
			...
			Expr: intstr.FromString(
				fmt.Sprintf(
					"count(kube_pod_status_phase{pod=~'virt-controller-.*', namespace='%s', phase='Running'} == 1) or vector(0)",
					namespace,
				),
			),
		},
		...
		},
		{
			...
			Expr: intstr.FromString(
				fmt.Sprintf(
					"count(kube_pod_status_phase{pod=~'virt-operator-.*', namespace='%s', phase='Running'} == 1) or vector(0)",
					namespace,
				),
			),
		},
		...
		},
		{
			...
			MetricType: operatormetrics.GaugeType,
			Expr: intstr.FromString(
				fmt.Sprintf("sum(up{pod=~'virt-handler-.*', namespace='%s'}) or vector(0)", namespace),
			),
		},
	}
}
